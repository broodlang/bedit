...
- [x] **P5 Removing a breakpoint can revert a later redefinition** (`debugger.blsp:548-575`); FIXED the revert, and any LOOP process (every window, every `--serve` client — `*ui-loop*`) now runs through a breakpoint, not only the first window. OPEN:
  other editor-internal processes (buffer processes, the registry) can still be paused. — FIXED: brood `proc/flag :no-break` (01fc3822) — a breakpoint passes such a process through; std sets it on every buffer process, the buffer registry and the evalsession worker, and bedit on its collab registry.
...
- [x] **X3 Global `def` caches** (`wrap.blsp:104`, `view.blsp:107`,
  `complete-at-point.blsp:59`). *Brood:* bounded memo / LRU. — FIXED: brood `std/memo` `defmemo` (61fb05a6) — the last few calls, matched by `=`. The row table, the bracket pair and the word list use it. Found on the way: the row cache was a MAP keyed on the rope, and a rope hashes by its whole text, so every lookup read the document; and the bracket cache ignored the theme, so a theme switch left the pair in the old colours until point moved (test in modes_test).
- [x] **X4 `model.blsp` names its feature clients.** *Brood:* `autoload` declarations. — FIXED without a language change: the three tables that named features (off-loop task replies, live-evaluating buffers, feature timers) are `defonce` registries each feature joins when it loads (`register-task-reply`, `register-live-eval-client`, `register-timer-client`). A deferred module is registered exactly when its rows can first matter, since a task or a live buffer only exists once its module has loaded.
